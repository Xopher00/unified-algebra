...
## Done: Parametric morphisms via Hydra lambda calculus (2026-05-15)

Route parameters are morphism-valued lexical variables, bound at compile time.

**Implemented:**
- `route f(x, y) = body` declares params; body contains `Ref("x")`, `Ref("y")`
- `f(a, b)` in body position → `MorphismApp` AST node (parser preserves structure, no substitution)
- Realization: `Ref(name)` → `TermVariable(Name(name))`, parameterized bodies wrapped in `TermLambda`, `MorphismApp` → curried `TermApplication`. Hydra beta-reduces.
- General morphism application: `g(a, b)` for non-parameterized g → lowered as `apply(g_term, paired_args)`
- Lexical params shadow ALL builtins (`x`, `id`, `copy`, `fst`, etc.)
- Duplicate param check, arity check, `validate_program` catches unresolved Refs
- `signature(Ref(name), frozenset({name}))` → `(TypeVariable(Name(name)), TypeVariable(Name(name)))`
- 315 tests passing

**Resolved ambiguities** (from the prior investigation list):
1. `_contextual_term` applies TermVariable as a function — correct because params are morphism-valued. `P.apply(TermVariable("x"), value)` reduces when x is bound to an actual morphism.
2. `T.term_lambda` emits `TermLambda` via `P.lam`. Scoping is standard lambda calculus.
3. `main.py` calls `lower()` → `realize()`. `validate_program` called after parse. No other changes.
4. `signature()` accepts `param_names: frozenset[str]`. Returns TypeVariable for declared params.
5. `TermLet(Let(bindings: frozenlist[Binding], body: Term))`. `Binding(name: Name, term: Term, type_scheme: Maybe[TypeScheme])`. Available but not needed for this path.
6. New AST node: `MorphismApp(fun, args, param_names)`. Parser records call structure; realize handles lowering.
7. TypeVariable tracked by `free_variables_in_type`. The existing `split_input`/`_mk_child_call` infrastructure activates when param != TypeUnit — TypeVariable on ContextualBinary.param would enable this.
8. Curried lambdas. Params bind as `λx.λy.body`; Hydra beta-reduces `(λx.λy.body)(a)(b)`.

## Next: Semantic elaboration pass for parameterized routes

The compile path (`compile_program` → `lower` → `realize`) currently runs with TypeUnit
placeholder types on ContextualBinary nodes. The semantic layer should fill in real types
before realization — including TypeVariable for route params. This would activate the existing
`split_input`/`_mk_child_call` param threading infrastructure during realization.

This belongs in the lowering pipeline (realize.py or a structure-layer helper that invokes
semantic analysis before calling realize). main.py stays out of it.

## Next: Arity > 1 application for backend ops

`multiply(x, y)` works NOW via `MorphismApp` with `param_names=()` — lowered as
`apply(multiply_term, pair(x_term, y_term))`. But backend ops could also be wrapped
in parameterized routes for the lambda-based path:
```
load torch
route mul(a, b) = a & b >> multiply
route f(x, y) = mul(x, y) >> linear
```
Both approaches coexist. The `MorphismApp` with empty param_names uses pairing + direct
application; parameterized routes use the lambda path.
